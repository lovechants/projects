...
impl Matrix {
    ...
    pub fn multiple(&mut self, other: &Matrix) -> Matrix {
        if self.cols != other.rows {
            panic!("Attempted multiplication of matrix of incorrect dimensions");
        }
    ...
    pub fn dot_product(&mut self, other: &Matrix) -> Matrix { 
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Attempted to apply dot dot product matrix of incorrect dimensions")
        }
